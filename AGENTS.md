...
**Stack:** TanStack Start (React 19 + TanStack Router + Vite 8) — SSR by default.
**Package manager:** npm (lockfile is `package-lock.json`; `.cursorrules` incorrectly says pnpm).
**Module:** ESM (`"type": "module"`), TypeScript 6 with `verbatimModuleSyntax` — must use `import type`.
**Path aliases:** `#/*` → `./src/*` (also `@/*`, but codebase uses `#/`).
...
| `npm run generate-routes` | Regenerate `src/routeTree.gen.ts` after adding/removing routes |
| `npm run db:generate` | Drizzle Kit — generate migration |
| `npm run db:migrate` | Drizzle Kit — apply migration |
| `npm run db:push` | Drizzle Kit — push schema directly to DB |
| `npm run db:studio` | Drizzle Kit — open DB GUI |
| `npm run preview` | Preview production build |

**Missing from package.json (run ad-hoc):**
- Type-check: `npx tsc --noEmit`
- No test framework, linter (ESLint), or formatter (Prettier) is configured yet.

## Route generation

Routes live in `src/routes/`. After creating/deleting a route file, run `npm run generate-routes` to regenerate `src/routeTree.gen.ts`. This file is auto-generated and marked readonly in `.vscode/settings.json` — do not edit manually.

## Auth

Better Auth with email/password. Handler at `src/routes/api/auth/$.ts` (catch-all splat route). Server config at `src/lib/auth.ts`. Client at `src/lib/auth-client.ts`. Uses `tanstackStartCookies` plugin for SSR-compatible session cookies.

## Database

PostgreSQL via Drizzle ORM. Schema in `src/db/schema.ts`. Client in `src/db/index.ts`. DB URL from `DATABASE_URL` env var (loads `.env.local` then `.env`). Migrations dir: `./drizzle/`.

## UI / styling

- **Tailwind CSS v4** via `@tailwindcss/vite` plugin.
- **Shadcn UI** (New York style, non-RSC, zinc base, Lucide icons). Install components via `npx shadcn@latest add <component>`. Aliases in `components.json`: `#/components`, `#/lib/utils`, `#/components/ui`.
- **TanStack Devtools** wired in `src/routes/__root.tsx` (Router + Query panels, positioned bottom-right).

## Vite config quirks

Plugin order matters in `vite.config.ts`: `devtools()` must be first, then `tailwindcss()`, `tanstackStart()`, `viteReact()`.

## Key files

| File | Purpose |
|---|---|
| `src/router.tsx` | Router factory — creates TanStack Router with SSR-Query integration |
| `src/routes/__root.tsx` | Root layout — document shell, devtools, head meta/links |
| `src/lib/auth.ts` | Better Auth server config |
| `src/db/schema.ts` | Drizzle schema |
| `drizzle.config.ts` | Drizzle Kit config (loads env from `.env.local`) |
